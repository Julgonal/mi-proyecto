...
exports.register = async (req, res) => {
  ...
  // Muestra el contenido del cuerpo de la solicitud para depuración 
  ...
  try { 
    ...
    // Se guarda el nuevo usuario en la base de datos await 
    newUser.save(); 
    ...
    if (err.name === 'ValidationError'){ 
      ...
      console.error(err); res.status(500).json({ message: 'Error en el servidor.', error: err.message });
      } 
  }
}; 
...
exports.login = async (req, res) => { 
  ...
  try { 
    ...
    console.error(err); 
    // Muestra errores de servidor en la consola 
